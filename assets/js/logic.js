...
let createClasseListeOf = (select_data) => {
    ...
    <div class="notees">${notes.map(note => `<div>${note}</div>`).join('')}</div>
    <div class="par">
...
let listerEtablisselents = () => {
    let etablissements = JSON.parse(SELECT_FROM('liste_etabs_add')) || []
...
function get_classe_infos() {
    ...
    dataClasse.map(m => {
        try {
            let noteR = m.notes
            let l = noteR[localStorage.getItem("decoup")].reduce((a, b) => a + b)
            mclass.push(l / noteR[localStorage.getItem("decoup")].length)
        } catch (e) {
